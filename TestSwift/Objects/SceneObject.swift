...
class SceneObject: NSObject, SCNSceneRendererDelegate, SCNPhysicsContactDelegate {
    // Game view
    var gameSCNView: GameSCNView = GameSCNView()
    ...
    #if os(OSX)
    internal var lastMousePosition = SIMD2<Float>(0)
    #elseif os(iOS)
    internal var padTouch: UITouch?
    internal var panningTouch: UITouch?
    #endif
    
    init(scnView: GameSCNView) {
        super.init()
        
        self.gameSCNView = scnView
        self.gameSCNView.sceneObject = self
        
        // Create a new scene.
        let scene = SCNScene(named: "game.scnassets/level.scn")!
        
        // Set the scene to the view and loop for the animation of the bamboos.
        gameSCNView.scene = scene
        gameSCNView.isPlaying = true
        gameSCNView.loops = true
    ...
    func panCamera(_ direction: SIMD2<Float>) {
        ...
        SCNTransaction.animateWithDuration(0.5, timingFunction: CAMediaTimingFunction(name: CAMediaTimingFunctionName.easeOut)) {
            ...
            self.cameraXHandle.rotation = SCNVector4(1,
                                                     0,
                                                     0,
                                                     (max(SCNFloat(-Double.pi  / 2),
                                                          min(0.13,
                                                              self.cameraXHandle.rotation.w + SCNFloat(directionToPan.y) * F))))
        }
    }
    ...
    private func characterDirection() -> SIMD3<Float> {
        ...
        if let pov = gameSCNView.pointOfView {
    ...
    private func setupCamera() {
        ...
        let pov = self.gameSCNView.pointOfView!
        ...
        gameSCNView.scene?.rootNode.addChildNode(cameraYHandle)
    ...
    private func setupAutomaticCameraPositions() {
        let rootNode = gameSCNView.scene!.rootNode
    ...
    private func setupSounds() {
        ...
        let node = self.gameSCNView.scene!.rootNode
    ...
    private var collectedPearlsCount = 0 {
        didSet {
            let gameSKScene: GameSKScene = gameSCNView.overlaySKScene as! GameSKScene
            gameSKScene.collectedPearlsCount = collectedPearlsCount
        }
    }
    ...
    private var collectedFlowersCount = 0 {
        didSet {
            let gameSKScene: GameSKScene = gameSCNView.overlaySKScene as! GameSKScene
            gameSKScene.collectedFlowersCount = collectedFlowersCount
    ...
    private func collectFlower(_ flowerNode: SCNNode) {
        if flowerNode.parent != nil {
            ...
            #if os(iOS) || os(tvOS)
            gameSCNView.scene!.addParticleSystem(collectFlowerParticleSystem, transform: particleSystemPosition)
            #elseif os(OSX)
            gameSCNView.scene!.addParticleSystem(collectFlowerParticleSystem, transform: particleSystemPosition)
            #endif
    ...
    private func showEndScreen() {
        ...
        // Stop the music.
        gameSCNView.scene!.rootNode.removeAllAudioPlayers()
        
        // Play the congrat sound.
        gameSCNView.scene!.rootNode.addAudioPlayer(SCNAudioPlayer(source: victoryMusic))

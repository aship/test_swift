...
class Slider: SKNode {
    var value: CGFloat = 0.0 {
        didSet {
            slider!.position = CGPoint(x: CGFloat(background!.position.x + value * width),
                                       y: CGFloat(0.0))
        }
    }
    ...
    init(width: Int, height: Int, text txt: String) {
        ...
        background = SKSpriteNode(color: SKColor.white,
                                  size: CGSize(width: CGFloat(width),
                                               height: CGFloat(2)))
        slider = SKShapeNode(circleOfRadius: CGFloat(height))
        slider!.fillColor = SKColor.white
        background!.anchorPoint = CGPoint(x: CGFloat(0.0),
                                          y: CGFloat(0.5))
        
        slider!.position = CGPoint(x: CGFloat(label!.frame.size.width / 2.0 + 15),
                                   y: CGFloat(0.0))
        background!.position = CGPoint(x: CGFloat(label!.frame.size.width / 2.0 + 15),
                                       y: CGFloat(0.0))
    ...
    override func mouseDragged(with event: NSEvent) {
        ...
        slider!.position = CGPoint(x: CGFloat(background!.position.x + pos),
                                   y: 0.0)
        value = pos / width
